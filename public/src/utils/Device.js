...
export class Device {
    ...
    testForMediaRecorderMediaStreamAPI() {
        if (!("MediaRecorder" in window) || !("MediaStream" in window)) {
            throw DEVICE_STATUS.unavailableMediaRecorderMediaStream;
        }
    }

    testForMimeTypesAndCodecs() {
        let foundType = false;
        let i = 0;
        let stream = new MediaStream();
        let recorder = null;
        while (i < AVAILABLE_MIME_TYPES.length) {
            try {
                if (MediaRecorder.isTypeSupported(AVAILABLE_MIME_TYPES[i])) {
                    recorder = new MediaRecorder(stream, { mimeType: AVAILABLE_MIME_TYPES[i] });
                    foundType = true;
                    break;
                }
            } catch (e) {
        ...
        if (!foundType) {
            throw DEVICE_STATUS.unavailableMimeType;
        }

        return AVAILABLE_MIME_TYPES[i];
    }
